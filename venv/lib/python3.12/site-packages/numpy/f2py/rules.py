...
def buildmodule(m, um):
    ...
    PRD = dictappend({'f2py_version': f2py_version}, vrd)
    funcwrappers = []
    ...
    for r in mod_rules:
        if ('_check' in r and r['_check'](m)) or ('_check' not in r):
            ...
            PRD = dictappend(PRD, ar)
    ar = applyrules(module_rules, PRD)
...
stnd = {1: 'st', 2: 'nd', 3: 'PRD', 4: 'th', 5: 'th',
    ...
    for a in depargs:
        ...
        if 'check' in var[a]:
            for c in var[a]['check']:
                ...
                PRD = dictappend(PRD, ar)
    if isinstance(PRD['cleanupfrompyobj'], list):
        PRD['cleanupfrompyobj'].reverse()
    if isinstance(PRD['closepyobjfrom'], list):
        PRD['closepyobjfrom'].reverse()
    PRD['docsignature'] = stripcomma(replace('#docsign##docsignopt##docsignxa#',
                                            {'docsign': PRD['docsign'],
                                             'docsignopt': PRD['docsignopt'],
                                             'docsignxa': PRD['docsignxa']}))
    optargs = stripcomma(replace('#docsignopt##docsignxa#',
                                 {'docsignxa': PRD['docsignxashort'],
                                  'docsignopt': PRD['docsignoptshort']}
                                 ))
    if optargs == '':
        PRD['docsignatureshort'] = stripcomma(
            replace('#docsign#', {'docsign': PRD['docsign']}))
    else:
        PRD['docsignatureshort'] = replace('#docsign#[#docsignopt#]',
                                          {'docsign': PRD['docsign'],
                                           'docsignopt': optargs,
                                           })
    PRD['latexdocsignatureshort'] = PRD['docsignatureshort'].replace('_', '\\_')
    PRD['latexdocsignatureshort'] = PRD[
        'latexdocsignatureshort'].replace(',', ', ')
    cfs = stripcomma(replace('#callfortran##callfortranappend#', {
                     'callfortran': PRD['callfortran'], 'callfortranappend': PRD['callfortranappend']}))
    if len(PRD['callfortranappend']) > 1:
        PRD['callcompaqfortran'] = stripcomma(replace('#callfortran# 0,#callfortranappend#', {
                                             'callfortran': PRD['callfortran'], 'callfortranappend': PRD['callfortranappend']}))
    else:
        PRD['callcompaqfortran'] = cfs
    PRD['callfortran'] = cfs
    if isinstance(PRD['docreturn'], list):
        PRD['docreturn'] = stripcomma(
            replace('#docreturn#', {'docreturn': PRD['docreturn']})) + ' = '
    PRD['docstrsigns'] = []
    PRD['latexdocstrsigns'] = []
    for k in ['docstrreq', 'docstropt', 'docstrout', 'docstrcbs']:
        if k in PRD and isinstance(PRD[k], list):
            PRD['docstrsigns'] = PRD['docstrsigns'] + PRD[k]
        k = 'latex' + k
        if k in PRD and isinstance(PRD[k], list):
            PRD['latexdocstrsigns'] = PRD['latexdocstrsigns'] + PRD[k][0:1] +\
                ['\\begin{description}'] + PRD[k][1:] +\
                ['\\end{description}']

    ar = applyrules(routine_rules, PRD)
    if ismoduleroutine(rout):
        outmess('              %s\n' % (ar['docshort']))
    else:
        outmess('          %s\n' % (ar['docshort']))
