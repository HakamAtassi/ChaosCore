...
def buildcallback(rout, um):
    ...
    PRD = dictappend({}, vrd)
    cb_map[um].append([rout['name'], PRD['name']])
    ...
    for a in depargs:
        ...
        for r in cb_arg_rules:
            ...
            if ('_check' in r and r['_check'](var[a])) or ('_check' not in r):
                ...
                if '_break' in r:
                    break
    if 'args' in PRD and 'optargs' in PRD:
        if isinstance(PRD['optargs'], list):
            PRD['optargs'] = PRD['optargs'] + ["""
#ifndef F2PY_CB_RETURNCOMPLEX
,
#endif
"""]
            PRD['optargs_nm'] = PRD['optargs_nm'] + ["""
#ifndef F2PY_CB_RETURNCOMPLEX
,
#endif
"""]
            PRD['optargs_td'] = PRD['optargs_td'] + ["""
#ifndef F2PY_CB_RETURNCOMPLEX
,
#endif
"""]
    ...
    if optargs == '':
        PRD['docsignature'] = stripcomma(
            replace('#docsign#', {'docsign': PRD['docsign']}))
    else:
        PRD['docsignature'] = replace('#docsign#[#docsignopt#]',
                                     {'docsign': PRD['docsign'],
                                      'docsignopt': optargs,
                                      })
    PRD['latexdocsignature'] = PRD['docsignature'].replace('_', '\\_')
    PRD['latexdocsignature'] = PRD['latexdocsignature'].replace(',', ', ')
    PRD['docstrsigns'] = []
    ...
    if 'args' not in PRD:
        ...
        PRD['args_td'] = ''
        PRD['args_nm'] = ''
    if not (PRD.get('args') or PRD.get('optargs') or PRD.get('strarglens')):
        PRD['noargs'] = 'void'

    ar = applyrules(cb_routine_rules, PRD)
    cfuncs.callbacks[PRD['name']] = ar['body']
    ...
    cfuncs.typedefs_generated[PRD['name'] + '_typedef'] = ar['cbtypedefs']
    ar['need'].append(PRD['name'] + '_typedef')
    cfuncs.needs[PRD['name']] = ar['need']

    capi_maps.lcb2_map[PRD['name']] = {'maxnofargs': ar['maxnofargs'],
