...
class SOC_TB:
    def __init__(self, dut, binary="/home/hakam/Repos/ChaosCore/binaries/bin/hello_world.bin"):
        ...
        with open(binary[0], "rb") as f:
            binary_data = f.read()
        for i, byte in enumerate(binary_data):
            self.axi_ram.write(i, byte.to_bytes(1, byteorder='little'))
 
        with open(binary[1], "rb") as f:
            binary_data = f.read()
        for i, byte in enumerate(binary_data):
            self.axi_ram.write(i+0x0100_0000, byte.to_bytes(1, byteorder='little'))

        
        # Optionally, dump a portion of memory to verify
        self.axi_ram.hexdump(0x0100_0000, len(binary_data), prefix="RAM")  # Dump up to 1KB or file size
