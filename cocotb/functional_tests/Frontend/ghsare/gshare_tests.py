...
@cocotb.test()
async def gshare_fuzzing_from_set(dut):
    random.seed(0x42)
    ...
    dut_state = dut.get_cache_state()
    model_state =gshare_model.get_state()
    assert model_state == dut_state, "states diverge"
...
@cocotb.test()
async def gshare_fuzzing_increment(dut):
    # count each addr from 0 -> 3, then 3->0
    random.seed(0x42)
